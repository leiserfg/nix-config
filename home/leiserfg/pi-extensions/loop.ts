...
async function selectSummaryModel(
  ...
  // Use gemini-3-flash for antigravity provider
  if (ctx.model.provider === "antigravity") {
    const geminiModel = ctx.modelRegistry.find(
      ctx.model.provider,
      "gemini-3-flash",
    );
    if (geminiModel) {
      const apiKey = await ctx.modelRegistry.getApiKey(geminiModel);
      if (apiKey) {
        return { model: geminiModel, apiKey };
      }
    }
  }
